...
public class ProductRepositoryWithBagRelationshipsImpl implements ProductRepositoryWithBagRelationships {
    ...
    Product fetchOrderRegistrationInfos(Product result) {
        return entityManager
            .createQuery(
                "select product from Product product left join fetch product.orderRegistrationInfos where product.id = :id",
                Product.class
            )
    ...
    List<Product> fetchOrderRegistrationInfos(List<Product> products) {
